...
exports.createUser = async function (_, { userInput }) {
    ...
    const { email, password, username, fullname } = userInput;
    const errors = [];
    ...
    if (!validator.isLength(username, { max: 15, min: 4 })) {
        errors.push({ message: "Username should be more than 3 characters and less than 16 characters" });
    }
    ...
    return { username: createdUser.username, fullname: createdUser.fullname, message: "Check your inbox for the confirmation link." };
};
...
exports.verifyEmail = async function (_, { confirmationToken }) {
    ...
    let confirmed = jwt.verify(confirmationToken, process.env.EMAIL_CONFIRMATION_TOKEN_SECRET);
    ...
    if (user.verified === true && user.email === confirmed.email) return "Your email have already been verified.";
...
exports.updateUser = async function (_, { bio, about, fullname, username }, context) {
...
exports.changePassword = async function (_, { password, newPassword }, context) {
...
exports.changeProfilePic = async (_, { file, id }, context) => {
...
exports.forgotPassword = async function (_, { confirmationToken, newPassword }) {

    const confirmed = jwt.verify(confirmationToken, process.env.CONFIRMATION_TOKEN_SECRET);
