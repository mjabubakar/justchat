...
exports.groupInfo = async function (_, { groupId }, context) {
    ...
    const userMember = await GroupMember.findOne({ where: { memberId: user.id, groupId } });
    ...
    return {
        info: group.info, members: groupMembers, createdBy: createdBy.username, createdOn: group
            .createdAt
    };
}
...
exports.groupMessages = async function (_, { id }, context) {
    ...
    const member = await GroupMember.findOne({ where: { groupId: id, memberId: user.id } });
    ...
    for (msg of messages) {
        ...
        groupMessages.push({ sentBy: user.username, time: functions.convertToTime(createdAt), type, message });
    }
...
exports.leaveGroup = async function (_, { groupId }, context) {
    ...
    const member = await GroupMember.findOne({ where: { groupId, memberId: user.id } });
...
exports.addMember = async function (_, { groupId, username }, context) {
    ...
    const member = await GroupMember.findOne({ where: { memberId: userM.id, groupId } });
    const userMember = await GroupMember.findOne({ where: { groupId, memberId: user.id } });
...
exports.updateType = async function (_, { groupId, username }, context) {
    ...
    return `${userM.username} is ${member.type === "admin" ? "no more an admin" : "now an admin"}`;
}

exports.createGroup = async function (_, { name, profilepic, members }, context) {
    ...
    for (member of members) {
        const check = await User.findOne({ where: { username: member["username"] } });
        if (!check) {
